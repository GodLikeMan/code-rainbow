...
<?php
			...
			$db->exec('CREATE TABLE  Products ( sku TEXT PRIMARY KEY , start_year INTEGER , end_year INTEGER ,title TEXT)');
		...
		while(($data = fgetcsv($handle,2500,","))!== FALSE){
			...
			*/
			...
			if(count($headerLine)===0){
				...
				$stmt = $db->prepare('INSERT INTO Products (sku , start_year , end_year , title) values (:sku,:s_year,:e_year,:title)');
				...
				for($i=0;$i<count($headerLine);$i++){
					if(($i!=0)&&($i!=4)&&($i!=6)){
						...
						$result = $db->query('SELECT id FROM Tags WHERE "'.$data[$i].'" = data');
						...
						if($arr = $result->fetchArray()){
							...
							$stmt = $db->prepare('INSERT INTO Tags (data) values (:data)');
							$stmt->bindParam(':data',$data[$i],SQLITE3_TEXT);
							$stmt->execute();	
							$lastInsertRowID = $db->lastInsertRowID();
